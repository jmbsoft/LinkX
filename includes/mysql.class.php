...
class DB
{
    ...
    function Row($query, $binds = array())
    {
        ...
        if( !$result )
        {
            trigger_error(mysql_error() . "<br />$query", E_USER_ERROR);
        }
    ...
    function Prepare($query, &$binds)
    {
        ...
        foreach( $pieces as $piece )
        {
            if( $piece == '?' )
            {
                if( $binds[0] === NULL )
                    $query_result .= 'NULL';
                else if( is_numeric($binds[0]) )
                    $query_result .= $binds[0];
                else
                    $query_result .= "'" . mysql_real_escape_string($binds[0], $this->handle) . "'";

                array_shift($binds);
            }
            else if( $piece == '#' )
            {
                $binds[0] = str_replace('`', '\`', $binds[0]);
                $query_result .= "`" . $binds[0] . "`";
                array_shift($binds);
            }
            else
