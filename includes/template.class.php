...
class Template
{
    ...
    function display($template, $cache_id = '', $readfile = FALSE)
    {
        ...
        if( $this->caching && $this->is_cached($template, $cache_id) )
        {
            if( $readfile )
            {
                readfile($this->cache_dir . "/$cache_id#$template");
            }
            else
            {
    ...
    function write_template($template, &$generated)
    {
        $fd = fopen($template, 'w');
        if( $fd  )
        {
...
function tnearest_half($number)
{
    return round($number * 2) / 2;
}

function trewrite($string)
{
    $string = preg_replace(array('~[^a-z0-9\-_]~i', '~-+~', '~-+$~'), array('-', '-', ''), $string);
