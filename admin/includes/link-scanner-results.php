...
function mailSelected()
{
    ...
    {
        alert('Please select at least one link to e-mail');
        return false;
    }

    return {href: 'index.php?&r=lxShMailLink&' + selected.serialize()};
}
...
function deleteSelected(id)
{
    ...
    if( confirm('Are you sure you want to delete ' + (selected.length > 1 ? 'the selected links?' : 'this link?')) )
    {
        infoBarAjax({data: 'r=lxDeleteLink&config_id=<?php echo urlencode($_REQUEST['config_id']); ?>&' + selected.serialize()});
    }

    return false;
}

function doToSelected(id, status)
{
    var selected = getSelected(id);

    if( selected.length < 1 )
    {
        alert('Please select at least one link to ' + what);
        return false;
    }

    if( confirm('Are you sure you want to do this for ' + (selected.length > 1 ? 'the selected links?' : 'this link?')) )
    {
        infoBarAjax({data: 'r=lxStatusLink&status='+status+'&' + selected.serialize()});
    }
  ...
  <div id="centered-content" class="max-width">
    <div class="heading">
      <div class="heading-icon">
        <a href="docs/link-scan-results.html" target="_blank"><img src="images/help.png" border="0" alt="Help" title="Help"></a>
      </div>
    ...
    <input type="hidden" name="r" value="lxShSearchScannerResults">
